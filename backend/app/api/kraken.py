...
class KrakenTestResponse(BaseModel):
    success: bool
    token_count: int = 0
    message: str = ""


class KrakenSyncRequest(BaseModel):
    api_key: str
    api_secret: str
    portfolio_id: str
...
@router.post("/test", response_model=KrakenTestResponse)
def test_kraken(
    body: KrakenTestRequest,
    current_user: User = Depends(get_current_user),
):
    """Test Kraken API credentials and preview balances."""
    try:
        result = test_kraken_connection(body.api_key, body.api_secret)
        return KrakenTestResponse(**result)
    except Exception as e:
        return KrakenTestResponse(success=False, message=str(e))


@router.post("/sync", response_model=SyncResponse)
def sync_kraken(
    body: KrakenSyncRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Import Kraken crypto holdings into a portfolio."""
    portfolio = get_portfolio(db, uuid.UUID(body.portfolio_id), current_user)
    result = import_kraken_positions(db, portfolio, body.api_key, body.api_secret)
